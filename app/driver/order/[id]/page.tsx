...
export default function DriverOrderOffer({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Stabilize handleReject with useCallback so it can safely be used in effects
  ...
  useEffect(() => {
    ...
    const fetchOrder = async () => {
      ...
      // Validasi kepemilikan tawaran
      if (orderData.offered_to_driver_id !== session.user.id || orderData.driver_id !== null) {
         toast.error("Tawaran sudah kadaluarsa atau diambil orang lain")
         return router.replace("/driver")
      }
      ...
      const expiresAt = new Date(orderData.offer_expires_at).getTime()
      const now = Date.now()
      const diff = Math.max(0, Math.floor((expiresAt - now) / 1000))
      
      if (diff === 0) {
        handleReject(true) // Auto reject if already expired
      } else {
        setTimeLeft(diff)
      }
  ...
  }, [orderId, router, handleReject])

  // Countdown effect — only runs once when loading is done
  useEffect(() => {
    if (loading) return
    if (timerRef.current) clearInterval(timerRef.current)
  ...
  }, [loading]) // Only start once when loading finishes
  ...
  // Calculate percentage for circular progress (based on initial full time)
         ...
         <div className="w-full bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-6 shadow-2xl mb-8">
             <div className="flex justify-between items-start mb-6 border-b border-white/10 pb-4">
                 <div>
                     ...
                     <h2 className="text-xl font-bold text-white leading-tight">{order.customer_name}</h2>
                 </div>
             ...
             <div className="space-y-4">
                 <div className="flex items-start gap-4">
                     ...
                     <div>
                         <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-0.5">Titik Jemput (Toko)</p>
