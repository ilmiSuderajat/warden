...
export default function EditAddressPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        const fetchAddressDetail = async () => {
            try {
                ...
                console.error(err)
                toast.error("Gagal memuat detail alamat")
                router.push("/address")
            } finally {
                setFetching(false)
            }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.latitude || !formData.longitude) {
            toast.error("Titik koordinat (Maps) belum terdeteksi. Silakan klik 'Deteksi Lokasi Saya' atau cari alamat.")
            return
        }
        setLoading(true)
        ...
        <div className="min-h-screen bg-slate-50 max-w-md mx-auto font-sans pb-10">
            ...
            <header className="fixed top-0 left-0 right-0 z-50 flex justify-center bg-white border-b border-slate-100">
                <div className="w-full max-w-md h-14 flex items-center px-4">
                    ...
                    <h1 className="ml-3 text-base font-bold text-slate-900">Ubah Alamat</h1>
                </div>
            </header>
            ...
            <div className="pt-20 px-5">
                ...
                <form onSubmit={handleSubmit} className="space-y-5 pb-10">
                    ...
                    >
                        {loading ? "Sedang Memperbarui..." : "Update Alamat Sekarang"}
                    </button>
                </form>
            </div>
        </div>
