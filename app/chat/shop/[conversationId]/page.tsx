...
// Skeleton untuk bubble chat saat loading
        ...
        >
          <div
            className={`h-10 rounded-2xl animate-pulse ${
              i % 2 === 0 ? "bg-white border border-slate-100 w-48" : "bg-indigo-100 w-36"
            }`}
...
export default function UnifiedChatRoom() {
  ...
  const convContextRef = useRef<ConversationContext | null>(null);
  ...
  // Ref untuk menandai apakah loadOlder sedang berjalan (mencegah double call dari observer)
  const isFetchingOlderRef = useRef(false);
  const hasOlderRef = useRef(false);

  // Product context (buyer attach produk ke pesan)
  ...
  const fetchInitial = useCallback(
    async (uid: string) => {
      try {
        // Fetch conversation + messages concurrently dalam 1 round-trip
        const [convRes, msgsRes] = await Promise.all([
          supabase
            .from("shop_conversations")
            .select("*, shops!inner(owner_id, name, image_url)")
        ...
        // Hanya fetch buyer name jika kita adalah shop owner
        let targetName = "";
        ...
        if (role === "buyer") {
          ...
          // Guard: buyer harus punya alamat terdaftar
          ...
          if (!buyerAddress) {
            toast.error("Tambahkan alamat terlebih dahulu untuk menggunakan fitur chat.", {
              description: "Hanya pengguna dengan alamat terdaftar yang bisa menghubungi toko.",
            });
            router.replace("/address/add");
            return;
          }
        } else {
          // Fetch nama pembeli (1 query kecil, hanya jika perlu)
        ...
        setConvContext(ctx);
        convContextRef.current = ctx;

        // Proses messages
        const msgs = ((msgsRes.data || []).reverse()) as ChatMessage[];
        allMessagesRef.current = msgs;
        oldestCursorRef.current = msgs[0]?.created_at ?? null;
        const totalCount = msgsRes.count ?? 0;
        setHasOlderOnServer(totalCount > FETCH_SIZE);
        hasOlderRef.current = totalCount > FETCH_SIZE;
        syncVisible(true);

        // Mark read — fire-and-forget, tidak perlu await
        supabase
          .from("shop_messages")
          .update({ is_read: true })
          .eq("conversation_id", conversationId)
    ...
    [conversationId, syncVisible, router]
  );

  // Setup realtime SEKALI — gunakan ref untuk convContext agar tidak recreate channel
  const setupRealtime = useCallback(
      ...
      const channel = supabase
        ...
        .on(
          ...
          (payload) => {
            ...
            if (newMsg.sender_id !== uid && isAtBottomRef.current) {
              const ctx = convContextRef.current;
              supabase
                .from("shop_messages")
                .update({ is_read: true })
                .eq("id", newMsg.id);
              if (ctx) {
                supabase
                  .from("shop_conversations")
                  .update(ctx.role === "buyer" ? { unread_buyer: 0 } : { unread_shop: 0 })
                  .eq("id", conversationId);
              }
            }
          }
        ...
        .on("broadcast", { event: "typing" }, (payload) => {
          if (payload.payload?.user_id !== uid) {
            ...
            if (payload.payload?.is_typing) setTimeout(() => setIsTyping(false), 3000);
          }
        })
        .subscribe();
    ...
    [conversationId, syncVisible]
    // convContext DIHAPUS dari deps — pakai convContextRef agar tidak recreate channel
  );
  ...
  const loadOlder = useCallback(async () => {
    if (isFetchingOlderRef.current || !hasOlderRef.current || !oldestCursorRef.current)
      return;
    ...
    if (data?.length) {
      ...
      const newHasOlder = older.length === FETCH_SIZE;
      setHasOlderOnServer(newHasOlder);
  ...
  // IntersectionObserver — stabil, tidak recreate setiap render
  useEffect(() => {
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    // Optimistic update — pesan langsung muncul
    ...
    try {
      // SATU RPC call = insert pesan + update conversation + increment unread (atomic)
      const { error } = await supabase.rpc("send_shop_message", {
        p_conversation_id: conversationId,
        p_sender_id: user.id,
        p_message: finalMessage,
        p_role: convContext.role,
      });
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-slate-50 z-[60]">
      <div className="flex flex-col flex-1 max-w-md mx-auto w-full overflow-hidden">
                ...
                <div className="ml-3 flex items-center gap-3">
                  <div className="w-9 h-9 bg-slate-100 rounded-full flex items-center justify-center overflow-hidden border border-slate-200 shrink-0">
                    {convContext.target_image ? (
                      <img
                        src={convContext.target_image}
                        alt={convContext.target_name}
                        className="w-full h-full object-cover"
                      />
                    ) : convContext.role === "buyer" ? (
        ...
        >
          {/* Sentinel untuk load older */}
          <div
            id="top-sentinel"
            className="w-full flex justify-center h-8 items-center"
          >
            {isFetchingOlder ? (
                            ...
                            if (msg.message.startsWith('{"type":"product_reference"')) {
                              ...
                              return (
                                <div className="flex flex-col gap-2">
                                  <div
                                    onClick={() =>
                                      router.push(`/product/${parsed.product.id}`)
                                    }
                                    className={`flex items-center gap-2 p-2 rounded-xl border ${
                                      isMine
                                        ? "bg-white/10 border-indigo-500 hover:bg-white/20"
                                        : "bg-slate-50 border-slate-200 hover:bg-slate-100"
                                    } cursor-pointer transition-colors active:scale-95`}
                                  >
                                    {parsed.product.image_url ? (
                                      <img
                                        src={parsed.product.image_url}
                                        alt={parsed.product.name}
                                    ...
                                    <div className="flex-1 min-w-0 pr-2">
                                      ...
                                      <p
                                        className={`text-[10px] font-bold ${
                                          isMine ? "text-indigo-100" : "text-indigo-600"
                                        }`}
                                      >
                                        Rp {parsed.product.price?.toLocaleString("id-ID")}
                                      </p>
                                    </div>
                                  </div>
        ...
        <div className="flex-none bg-white border-t border-slate-200">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-2.5 flex items-center gap-3 pr-8 shadow-sm">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-indigo-600 font-bold">
                    Rp {productContext.price?.toLocaleString()}
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => setProductContext(null)}
                  className="absolute right-6 p-1.5 text-slate-400"
                >
          ...
          <form onSubmit={sendMessage} className="p-4 flex gap-3">
            ...
            <button
              ...
              className="px-5 bg-indigo-600 text-white rounded-2xl font-bold flex items-center gap-2 hover:bg-indigo-700 disabled:bg-slate-300 shadow-md transition-colors"
            >
              {sending ? (
