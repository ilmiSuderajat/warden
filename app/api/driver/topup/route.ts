...
export async function POST(req: Request) {
    try {
        ...
        if (userError || !userData || userData.role !== "driver") {
            return NextResponse.json({ error: "Driver tidak valid." }, { status: 404 })
        }

        const userIdShort = userData.id.replace(/-/g, "").substring(0, 8)
        const midtransOrderId = `DRVTOPUP-${userIdShort}-${Date.now()}`
        ...
        if (insertError || !topupReq) {
            console.error("[Driver Topup] Insert error:", insertError)
        ...
        const parameter = {
            ...
            item_details: [
                {
                    id: "topup-saldo-driver",
                    price: amount,
                    quantity: 1,
                    name: `Topup Saldo Driver Warden`,
                },
            ],
        }
        ...
        try {
            ...
            console.error("[Driver Topup Midtrans Error]:", midError.message)
