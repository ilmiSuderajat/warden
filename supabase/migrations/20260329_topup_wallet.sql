CREATE OR REPLACE FUNCTION topup_wallet(p_amount NUMERIC)
RETURNS BOOLEAN AS $$
DECLARE
  v_balance NUMERIC;
BEGIN
  ...
  IF p_amount <= 0 THEN
    RAISE EXCEPTION 'Topup amount must be greater than 0';
  END IF;
  ...
  PERFORM create_transaction(
    auth.uid(), NULL, 'topup', p_amount,
    'Wallet topup', NULL
  );

  PERFORM _write_audit_log(
    auth.uid(), 'user', 'topup_wallet', NULL,
    jsonb_build_object(
      'amount', p_amount,
